...
%----------------------------------------------------------------------------
function [y,out,h_next] = Runge_Kutta_Fehlberg_4_5_KC(func,y0,x,h,xmax,tolerance,Kij,N,k,Aij,omega)
...
while (x < xmax)
      scale = 1.0;
      for i = 1:ATTEMPTS
         [temp_y, err] = Runge_Kutta(func, temp_y0, x, h, Kij, N, k, Aij, omega);
         err = norm(err);
         if (err == 0.0)
...
%----------------------------------------------------------------------------
function [y, out] = Runge_Kutta(func, y, x0, h, Kij, N, k, Aij, omega)
...
k1 = func(x0, y, Kij,N, k, Aij, omega);
k2 = func(x0+h2, y + h * b21 * k1, Kij,N, k, Aij, omega);
k3 = func(x0+h3, y + h * ( b31*k1 + b32*k2), Kij,N, k, Aij, omega );
k4 = func(x0+h4, y + h * ( b41*k1 + b42*k2 + b43*k3), Kij,N, k, Aij, omega );
k5 = func(x0+h,  y + h * ( b51*k1 + b52*k2 + b53*k3 + b54*k4), Kij,N, k, Aij, omega );
k6 = func(x0+h6, y + h * ( b61*k1 + b62*k2 + b63*k3 + b64*k4 + b65*k5), Kij,N, k, Aij, omega );
